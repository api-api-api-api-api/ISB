 ...
 *
 */
class Dpfbrngen extends AppModel {
...
function maxNoDPFGenerator($pref,$thn,$bln){
	$pref=$pref.$thn.$bln;
	$maxRec=$this->find('all', array(
				'conditions' => array(
					'substring(noDPF,1,'.strlen($pref).')' => $pref
				),
				'fields' => array('max(noDPF) noDPF')
			));
	$jmlRec=count($maxRec);
	
	if($maxRec==0){$maxId=$prefId."0001";}
	else{
		$maxId=$maxRec[0][0]["noDPF"]; 
...
public function saveDataDPF2($noNota,$tanggal,$userId,$arcoId,$namaTP,$periode,$kodeDist,$distributor,$comId,$outletId,$keterangan,$dmId,$smId,$gsmId,$finId,$buffer,$isAllNew,$pejId,$divisi,$subDivisi){
...
public function prosesApproval($userId,$userName,$groupId,$noDPF,$statusApproval,$keteranganApproval,$idRejectCols){
		...
		try{					 
			...
			if($statusApproval=='setuju'){
				...
				$dataDPF=$this->getDataDPFApproval('noDPF',$noDPF,$groupId);
...
public function cekCreateCsv($noDPF){
	/*if(trim($bulan)<>''){
	 $bulan=intval($bulan);
	 }*/
