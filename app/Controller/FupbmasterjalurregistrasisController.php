...
class FupbmasterjalurregistrasisController extends AppController {
    ...
    public function getData(){
        ...
        $txtJalurRegistrasi=$_POST['txtJalurRegistrasi'];
        ...
        $sql="SELECT * FROM dpfdplnew.masterjalurregistrasis WHERE jalurRegistrasi like '%$txtJalurRegistrasi%'  ORDER BY jalurRegistrasi";
           ...
           $txtData.="
               <tr>
                ...
                foreach($rsTampil as $data){
                    $id=$data['masterjalurregistrasis']['id'];
                    ...
                    $txtData.="<tr>
                        ...
                        <td style='vertical-align:middle;text-align:left;' id='txtJalurRegistrasi".$id."' >
                            <label id='lbl".$id."' style='margin-bottom:unset;font-weight:unset;'>".$data['masterjalurregistrasis']['jalurRegistrasi']."</label> 
                            <input type='text' class='form-control' name='idJalurRegistrasi' id='idJalurRegistrasi".$id."' value='".$data['masterjalurregistrasis']['jalurRegistrasi']."' style='display:none;font-size:12px;'>
                        </td>
    ...
    public function saveJalurregistrasi(){
        $this->autoRender = false;
        $this->loadModel('User');
        ...
        try{
            ...
            $CRUD=$_POST['CRUD'];
            if($CRUD=='create'){
                $jalurRegistrasi=$_POST['jalurRegistrasi'];           
                $sql="INSERT INTO  dpfdplnew.masterjalurregistrasis (jalurRegistrasi)VALUES('$jalurRegistrasi')";
                //var_dump($simpanJenis);exit();
                $this->User->query($sql);

                $querygetID=$this->User->query("SELECT m.id FROM dpfdplnew.masterjalurregistrasis m  ORDER BY m.id DESC limit 1");
                $recordid=0;
                $id=$querygetID[0]['m']['id'];
                $query=$this->User->query("SELECT * FROM dpfdplnew.masterjalurregistrasis m  ORDER BY m.jalurRegistrasi");
                ...
                $id=$_POST['id'];
                $jalurRegistrasi=$_POST['jalurRegistrasi'];           
                $sql="UPDATE dpfdplnew.masterjalurregistrasis SET jalurRegistrasi='$jalurRegistrasi' WHERE id = '$id'";
                 $this->User->query($sql);
            }else{
    ...
    public function deleteJalurregistrasi(){
        $this->autoRender = false;
        $this->loadModel('User');
        ...
        try{
            ...
            $limit=10;
                ...
                //echo $sum1;exit();
                $this->User->query("DELETE	FROM dpfdplnew.masterjalurregistrasis WHERE id='$id'");

                $query2=$this->User->query("SELECT * FROM dpfdplnew.masterjalurregistrasis m  ORDER BY m.jalurRegistrasi");
